// randomtestcard3.c tests function tributeEffect
...
int tributeTest(struct gameState* state, int currentPlayer) {
	int numErrors = 0;
	struct gameState pre;
	memcpy (&pre, state, sizeof(struct gameState));

	int nextPlayer;
	if (currentPlayer == (state->numPlayers - 1)) { // Wrap around to 0th position
		nextPlayer == 0;
	}
	else {
